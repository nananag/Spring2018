...
API_KEY = 'AIzaSyBlJvxmLSs5AOSiV6e0x6gmy70jIlTtQsw'
...
TABLE_ID = '1pEl2AiHwIrKSFTuwgCCnqqkGLU0AcZPPI1NDDNUE'

# This is the default columns for the query
query_cols = []
query_values = ['United States'] #Change to be the value(s) you're querying in the column you've specified
...
def get_all_data(query):
    ...
    query = "SELECT * FROM " + TABLE_ID + " WHERE Origin = 'United States' "
    response = service.query().sql(sql=query).execute()
    logging.info(response['columns'])
    logging.info(response['rows'])
    return response
...
def make_query(cols, values, limit):
    ...
    query = "SELECT " + string_cols + " FROM " + TABLE_ID + " WHERE Origin = '" + string_values + "'"
